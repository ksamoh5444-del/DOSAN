...
export function InstallPrompt() {
  ...
  return (
    <div className="animate-toast-in fixed bottom-[calc(11rem+env(safe-area-inset-bottom))] left-1/2 z-40 w-[calc(100%-1.5rem)] max-w-sm -translate-x-1/2">
      <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/95 p-3 pe-2 shadow-xl backdrop-blur dark:border-white/10 dark:bg-slate-800/95">
        <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-indigo-500/10 text-indigo-500">
          <SquarePlus size={20} />
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-slate-900 dark:text-white">أضف دوسان إلى الشاشة الرئيسية</p>
          {deferredPrompt ? (
            <p className="text-xs text-slate-500 dark:text-slate-400">ثبّت التطبيق للوصول السريع بملء الشاشة.</p>
          ) : (
            <p className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400">
              اضغط <Share size={13} className="inline" /> ثم "إضافة إلى الشاشة الرئيسية"
            </p>
          )}
        </div>
        {deferredPrompt && (
